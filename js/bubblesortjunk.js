...
function setup(){
    ...
    background(20);
    sector = line_w + 2;
    v_size = width / sector - 2; 
    // a sector must be bigger than the line width in order to have a space betweel lines
    // -2 is for letting a space between canvas and lines
    vector = [v_size];
...
function draw(){
  ...
  for(let l = 0; l < v_size; l++){
    ...
    strokeWeight(line_w);
    line(l*sector + line_w/2 + line_w, height - line_w, l*sector + line_w/2 + line_w, height - vector[l] - line_w);
  }
}
